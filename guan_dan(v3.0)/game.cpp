...
void Game::tribute() {
    ...
    int firstPlayerIndex = -1, fourthPlayerIndex = -1;
    int currentIndex = 0;
    ...
    for (auto& player : players) {
        if (player->getARoundRank() == First) {
            firstPlayer = player;
            firstPlayerIndex = currentIndex;
        }
        if (player->getARoundRank() == Fourth) {
            fourthPlayer = player;
            fourthPlayerIndex = currentIndex;
        }
        currentIndex++;
    }
    ...
    // 显示上贡界面
    Card tributeCard = maxCard;

    // 如果是人类玩家，则显示上贡选择界面
    if (dynamic_cast<HumanPlayer*>(fourthPlayer.get())) {
        tributeCard = graphical.handleTributeSelection(fourthPlayer);
    }

    // 执行上贡动画
    graphical.animateCardTransfer(fourthPlayerIndex, firstPlayerIndex, tributeCard, true);

    // 显示上贡界面
    graphical.drawTributeInterface(fourthPlayer, firstPlayer, tributeCard);

    // 处理上贡卡牌传输
    fourthPlayer->removeCard(tributeCard);
    firstPlayer->addCard(tributeCard);
...
void Game::aRoundGame() {
    ...
    // 更新当前游戏级别
    currentLevel = firstPlayer->getLevel();
...
void Game::resetRoundGame() {
    ...
    for (int i = 0; i < 4; i++) {
        graphical.clearLastPlay(i); // 清除每个玩家的上次出牌记录
    }
}
...
void Game::settleScore() {
    ...
    for (const auto& p : players) {
        if (p->getARoundRank() == First) { // 找到第一名的队伍
